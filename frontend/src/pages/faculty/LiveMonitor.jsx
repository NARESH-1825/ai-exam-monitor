...
const vcBgBrd = (n) =>
  n >= 3
    ...
    : n >= 2
      ...
      : "bg-white/4 border-slate-700/20";

const statusBadge = (status, cheated, reason) => {
  if (cheated)
    return {
      label: "🚫 Cheated",
      cls: "bg-red-900/50 text-red-300 border border-red-800/40",
    };
  if (status === "submitted" && reason?.includes("Faculty"))
    return {
      label: "Blocked",
      cls: "bg-orange-900/50 text-orange-200 border border-orange-800/40",
    };
  if (status === "submitted" && reason?.includes("Time"))
    return {
      label: "Time Up",
      cls: "bg-yellow-900/50 text-yellow-200 border border-yellow-800/40",
    };
  if (status === "submitted")
    return {
      label: "Submitted",
      cls: "bg-blue-900/50 text-blue-300 border border-blue-800/40",
    };
  return {
    label: "● Attending",
    cls: "bg-green-900/50 text-green-400 border border-green-800/40 live-badge",
  };
};
...
const StudentCard = React.memo(({ s }) => {
  ...
  const violationSummary = (s.events || []).reduce((acc, e) => {
  ...
  return (
    <div
      ref={ref}
      className={`bg-gray-800/60 rounded-2xl border-2 transition-all ${
        s.cheated
          ? "border-red-600/50"
          : vc >= 2
    ...
    >
      ...
      <div className="px-3 pb-3 space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`rounded-lg px-2.5 py-1 border text-center ${vcBgBrd(vc)}`}
          >
        ...
        {s.status === "submitted" && s.percentage !== undefined && (
          <div className="bg-white/4 rounded-lg py-1.5 text-center">
            <span
              className={`font-bold text-sm ${s.cheated ? "text-red-400" : s.passed ? "text-green-400" : "text-orange-400"}`}
            >
        ...
        {open && Object.keys(violationSummary).length === 0 && (
          <p className="text-xs text-green-400 border-t border-slate-700/20 pt-2">
            ✅ No violations
          </p>
        )}
      </div>
...
const Leaderboard = ({ examId }) => {
  ...
  useEffect(() => {
    api
      .get(`/faculty/exam-results/${examId}`)
      .then(({ data }) => {
        setData(data);
        setLoading(false);
      })
  ...
  const bucketCounts = buckets.map((_, i) => {
    ...
    return submissions.filter(
      (s) =>
        !s.cheated && (s.percentage || 0) > lo && (s.percentage || 0) <= hi,
    ).length;
  });
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div
            key={i}
            className="bg-white/4 border border-slate-700/20 rounded-xl p-3 text-center"
          >
      ...
      <div className="space-y-2">
        {filtered.map((s) => {
          ...
          const vc = s.violationCount || 0;
          const vSummary = (s.violationEvents || []).reduce((acc, e) => {
            acc[e.type] = (acc[e.type] || 0) + 1;
            return acc;
          }, {});
          return (
            <div
              key={s.id}
              className={`bg-gray-800/50 rounded-xl border overflow-hidden ${s.cheated ? "border-red-800/40" : s.rank <= 3 ? "border-yellow-800/40" : "border-slate-700/20"}`}
            >
              <button
              ...
              >
                <span className="text-xl w-8 shrink-0 text-center">
                  {s.cheated ? "🚫" : medal(s.rank)}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-white text-sm">
                    {s.studentInfo?.name || "Unknown"}
                  </p>
                ...
                <div className="text-right shrink-0">
                  <p
                    className={`font-bold text-sm ${s.cheated ? "text-red-400" : s.passed ? "text-green-400" : "text-red-400"}`}
                  >
                    {s.cheated
                      ? `0/${totalMarks}`
                      : `${s.score ?? "—"}/${totalMarks}`}
                ...
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-lg ${vcBgBrd(vc)} ${vcColor(vc)}`}
                >
                  {vc}/3
                </span>
                    ...
                    {[
                      {
                        label: "Score",
                        val: s.cheated
                          ? `0/${totalMarks} (Cheated)`
                          : `${s.score ?? "—"}/${totalMarks} (${s.percentage ?? "—"}%)`,
                        color: s.cheated ? "text-red-400" : "",
                      },
                      {
                        label: "Submission",
                        val: s.autoSubmitted ? "Auto" : "Manual",
                        color: s.autoSubmitted
                          ? "text-orange-400"
                          : "text-blue-400",
                      },
                      {
                      ...
                      <div key={i} className="bg-white/4 rounded-lg p-2">
                        ...
                        <p
                          className={`text-xs font-medium ${c.color || "text-white"}`}
                        >
                          {c.val}
                        </p>
                      </div>
...
const PageCloseTimer = ({ startTime, pageCloseTime, onExpire }) => {
  ...
  return (
    ...
    >
      {secs === 0
        ? "Page Closed"
        : `Closes ${String(m).padStart(2, "0")}:${String(s).padStart(2, "0")}`}
    </span>
  );
};
...
const LiveMonitor = () => {
  ...
  const loadSubmissions = useCallback(() => {
    api
      .get(`/monitor/${examId}/submissions`)
      .then(({ data }) => {
        const map = {};
        (data.submissions || []).forEach((sub) => {
          const si =
            sub.studentInfo ||
            (typeof sub.student === "object" ? sub.student : null);
          const sid =
            si?.id || (typeof sub.student === "string" ? sub.student : null);
          if (!sid) return;
          map[sid] = {
            id: sub.id,
            student: si,
            ...
            autoSubmitReason: sub.autoSubmitReason,
        ...
        setStudents((prev) => {
          ...
          Object.keys(prev).forEach((sid) => {
            if (merged[sid]) {
              if (
                (prev[sid].events?.length || 0) >
                (merged[sid].events?.length || 0)
              )
                merged[sid].events = prev[sid].events;
              if (
                (prev[sid].violationCount || 0) >
                (merged[sid].violationCount || 0)
              )
                merged[sid].violationCount = prev[sid].violationCount;
            }
          });
          return merged;
        });
  ...
  useEffect(() => {
    api.get(`/exam/${examId}`).then(({ data }) => {
      setExam(data.exam);
      if (data.exam?.status === "completed") setActiveTab("results");
    });
    loadSubmissions();
      ...
      setStudents((prev) => {
        ...
        return {
          ...
          [studentId]: {
            ...
            events: [...(ex.events || []), event].slice(-50),
          },
        };
      });
    });
          ...
          [studentId]: {
            ...
            violationCount:
              violationCount !== undefined
                ? Math.max(violationCount, prev[studentId]?.violationCount || 0)
                : prev[studentId]?.violationCount,
          },
        }));
      },
    );
    ...
    <div className="flex items-center gap-2 flex-wrap">
      {exam?.status === "live" && exam.pageCloseTime > 0 && (
        <PageCloseTimer
          ...
          pageCloseTime={exam.pageCloseTime}
          onExpire={doEndExam}
        />
      )}
      ...
      <span className="text-xs bg-blue-900/25 text-blue-400 border border-blue-800/40 px-2.5 py-1 rounded-lg">
        ✓ {doneCount} Done
      </span>
      {cheatCount > 0 && (
        <span className="text-xs bg-red-900/30 text-red-400 border border-red-800/40 px-2.5 py-1 rounded-lg">
          🚫 {cheatCount}
        </span>
      )}
      {exam?.status === "live" && (
    ...
    <DashboardLayout title={pageTitle} actions={topActions}>
      ...
      {activeTab === "results" && isCompleted ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-400 text-center">
          <div className="w-16 h-16 rounded-2xl bg-gray-800/50 border border-slate-700/20 flex items-center justify-center text-3xl mb-4">
            👥
          </div>
          <h3 className="text-white font-semibold mb-1">No students yet</h3>
          <p className="text-sm text-gray-500">
            Students appear here as they join the exam
          </p>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-gray-400 text-sm">
            {doneCount} student{doneCount !== 1 ? "s" : ""} completed. End the
            exam to view results.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {liveEntries.map(([id, s]) => (
            <StudentCard key={id} s={s} />
          ))}
        </div>
      )}
    </DashboardLayout>
