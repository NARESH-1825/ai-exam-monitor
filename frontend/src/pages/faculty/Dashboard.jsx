...
const PageCloseTimer = ({ startTime, pageCloseTime }) => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      const r = Math.max(0, Math.floor((closeMs - Date.now()) / 1000));
      setSecs(r);
      return r;
    };
    if (calc() <= 0) return;
...
const statusConfig = {
  draft: {
    cls: "bg-gray-700/70 text-gray-300 border border-slate-600/30",
    dot: "bg-gray-400",
    extra: "",
  },
  scheduled: {
    cls: "bg-yellow-900/40 text-yellow-300 border border-yellow-800/40 scheduled-badge",
    dot: "bg-yellow-400",
    extra: "",
  },
  live: {
    cls: "bg-green-900/40 text-green-400 border border-green-800/40 live-badge",
    dot: "bg-green-400 pulse-dot live-ring",
    extra: "",
  },
  completed: {
    cls: "bg-blue-900/40 text-blue-400 border border-blue-800/40",
    dot: "bg-blue-400",
    extra: "",
  },
};
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-medium ${cfg.cls}`}
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${cfg.dot} ${status === "live" ? "pulse-dot" : ""}`}
      ></span>
      {status}
    </span>
  );
...
const FacultyDashboard = () => {
  ...
  const handleEnd = async (examId) => {
    ...
    if (!confirm("End this exam? All ongoing students will be auto-submitted."))
      return;
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to end exam", {
        className: "custom-toast",
        bodyClassName: "custom-toast-body",
      });
    }
  ...
  const statCards = [
    ...
    {
      label: "Live Now",
      val: exams.filter((e) => e.status === "live").length,
      icon: "🟢",
    ...
    {
      label: "Completed",
      val: exams.filter((e) => e.status === "completed").length,
      icon: "✅",
  ...
  const quickActions = [
    {
      ...
      icon: "📚",
      label: "Question Bank",
      desc: "Manage MCQ papers",
    },
    {
      to: "/faculty/exam-config",
      icon: "⚙️",
      label: "Create Exam",
      desc: "Configure & schedule",
    },
    {
      to: "/faculty/students",
      icon: "👥",
      label: "View Students",
      desc: "Manage enrolled students",
    },
  ];
    ...
    <DashboardLayout title="Faculty Dashboard" actions={header}>
      ...
      <div className="bg-gradient-to-r from-purple-900/35 to-blue-900/25 border border-purple-800/20 rounded-2xl p-5 mb-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p className="text-purple-200 text-sm mt-0.5">
            Manage your exams, questions, and monitor students
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        {statCards.map((s, i) => (
          <div
            key={i}
            className={`bg-gradient-to-br ${s.bg} rounded-xl p-4 text-center border ${s.border}`}
          >
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {quickActions.map(({ to, icon, label, desc }) => (
          <Link
            key={to}
            to={to}
            className="bg-gray-800/50 hover:bg-gray-800 border border-slate-700/20 hover:border-slate-600/40 rounded-xl p-4 text-center transition-all group hover:-translate-y-0.5"
          >
      ...
      <div className="bg-gray-800/50 border border-slate-700/20 rounded-2xl p-5">
        ...
        {loading ? (
          ...
          <div className="text-center py-10 text-gray-400">
            ...
            <p className="text-sm">
              {filter === "all" ? "No exams yet." : `No ${filter} exams.`}
            </p>
            {filter === "all" && (
              <Link
                to="/faculty/exam-config"
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-gray-500 text-xs">
                      ...
                      {exam.pageCloseTime > 0 && (
                        <span className="text-purple-400">
                          🏫 {exam.pageCloseTime}min
                        </span>
                      )}
