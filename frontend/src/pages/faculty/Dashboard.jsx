...
const PageCloseTimer = ({ startTime, pageCloseTime }) => {
  ...
  return (
    <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${
      ...
      secs < 300 ? 'text-red-300 bg-red-900/30 border border-red-800' : 'text-purple-300 bg-purple-900/30 border border-purple-800'}`}>
...
/* ── Status badge ─────────────────────────────────────────────── */
const statusConfig = {
  draft:     { cls: 'bg-gray-700/70 text-gray-300 border border-slate-600/30',                     dot: 'bg-gray-400',   extra: '' },
  scheduled: { cls: 'bg-yellow-900/40 text-yellow-300 border border-yellow-800/40 scheduled-badge', dot: 'bg-yellow-400', extra: '' },
  live:      { cls: 'bg-green-900/40 text-green-400 border border-green-800/40 live-badge',          dot: 'bg-green-400 pulse-dot live-ring', extra: '' },
  completed: { cls: 'bg-blue-900/40 text-blue-400 border border-blue-800/40',                       dot: 'bg-blue-400',   extra: '' },
};
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-medium ${cfg.cls}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${cfg.dot} ${status === 'live' ? 'pulse-dot' : ''}`}></span>
      {status}
    </span>
  );
};
...
const FacultyDashboard = () => {
  ...
  const handleLaunch = async (examId) => {
    if (!examId) { toast.error('Invalid exam ID'); return; }
    try {
  ...
  const handleEnd = async (examId) => {
    ...
    try {
      ...
      toast.success('Exam ended. Students auto-submitted.');
    } catch (err) { toast.error(err.response?.data?.message || 'Failed to end exam'); }
  };
    ...
    <DashboardLayout title="Faculty Dashboard" actions={header}>
      ...
      <div className="bg-gradient-to-r from-purple-900/35 to-blue-900/25 border border-purple-800/20 rounded-2xl p-5 mb-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex gap-2">
          <Link to="/faculty/exam-config" className="px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-xl text-sm font-semibold transition-all">
            ⚙️ New Exam
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {quickActions.map(({ to, icon, label, desc }) => (
          <Link key={to} to={to} className="bg-gray-800/50 hover:bg-gray-800 border border-slate-700/20 hover:border-slate-600/40 rounded-xl p-4 text-center transition-all group hover:-translate-y-0.5">
      ...
      <div className="bg-gray-800/50 border border-slate-700/20 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
          ...
          <div className="flex bg-gray-900/60 rounded-lg p-0.5 gap-0.5 border border-slate-700/20">
            {['all', 'draft', 'live', 'completed'].map(f => (
        ...
        {loading ? (
          ...
          <div className="text-center py-10 text-gray-400">
            ...
            <p className="text-sm">{filter === 'all' ? 'No exams yet.' : `No ${filter} exams.`}</p>
            {filter === 'all' && <Link to="/faculty/exam-config" className="text-blue-400 text-sm hover:underline mt-1 inline-block">Create one →</Link>}
          </div>
        ) : (
          <div className="space-y-2.5">
            {filteredExams.map(exam => {
              ...
              return (
                <div key={examId} className="flex items-start justify-between bg-white/[0.025] hover:bg-white/4 border border-slate-700/20 rounded-xl px-4 py-3.5 gap-4 transition-colors">
                  ...
                  <div className="flex gap-2 shrink-0 flex-wrap justify-end">
                    {exam.status === 'draft' && (
                      <button onClick={() => handleLaunch(examId)}
                    ...
                    {exam.status === 'live' && (
                      <>
                        ...
                        <button onClick={() => handleEnd(examId)}
