...
const PROCTOR_KEYS = ['faceDetection','eyeTracking','noiseDetection','objectDetection','tabSwitchLock','fullScreenForce'];
const DEFAULT_PROC = Object.fromEntries(PROCTOR_KEYS.map(k => [k, false]));
const DEFAULT_FORM = { title: '', description: '', duration: 60, pageCloseTime: 0, passingScore: 40, proctoring: { ...DEFAULT_PROC } };
...
const ExamConfig = () => {
  ...
  const [form, setForm] = useState(() => {
    try { const s = sessionStorage.getItem(STORAGE_KEY); if (s) return JSON.parse(s).form || DEFAULT_FORM; }
    catch {} return DEFAULT_FORM;
  });
  const [selectedPaper, setSelectedPaper] = useState(() => {
    try { const s = sessionStorage.getItem(STORAGE_KEY); if (s) return JSON.parse(s).selectedPaper || null; }
    catch {} return null;
  });
  ...
  useEffect(() => {
    sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ form, selectedPaper }));
  }, [form, selectedPaper]);

  const toggle     = key => setForm(p => ({ ...p, proctoring: { ...p.proctoring, [key]: !p.proctoring[key] } }));
  const enableAll  = ()  => setForm(p => ({ ...p, proctoring: Object.fromEntries(PROCTOR_KEYS.map(k => [k, true])) }));
  const disableAll = ()  => setForm(p => ({ ...p, proctoring: { ...DEFAULT_PROC } }));

  const activeCount = PROCTOR_KEYS.filter(k => form.proctoring[k]).length;
  const needsCam    = form.proctoring.faceDetection || form.proctoring.eyeTracking || form.proctoring.objectDetection;
  const needsMic    = form.proctoring.noiseDetection;

  const handleSubmit = async e => {
    e.preventDefault();
    if (!selectedPaper) { toast.error('Please select a question paper'); return; }
    if (!form.title.trim()) { toast.error('Exam title is required'); return; }
    if (form.duration < 1) { toast.error('Duration must be at least 1 minute'); return; }
    setSubmitting(true);
    try {
      const payload = {
        ...form,
        ...
        questions: (selectedPaper.questions || []).map(q => ({ ...q, _id: q.id || q._id || Math.random().toString(36).slice(2) })),
  ...
  const clearActions = (
    <button onClick={() => { sessionStorage.removeItem(STORAGE_KEY); setForm(DEFAULT_FORM); setSelectedPaper(null); toastFired.current = false; toast.info('Draft cleared'); setStep(0); }}
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl w-full mx-auto">
        ...
        {step === 0 && (
          <div className="bg-gray-800/50 border border-slate-700/20 rounded-2xl p-6 space-y-4 fade-in">
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block font-medium">Exam Title *</label>
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block font-medium">Description <span className="text-gray-600">(optional)</span></label>
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block font-medium">Passing Score (%)</label>
            ...
            <button type="button" onClick={() => setStep(1)} className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 rounded-xl text-sm font-semibold transition-colors">
              Next: Timer Settings →
            </button>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="bg-gray-800/60 border border-white/8 rounded-2xl p-6 space-y-4 fade-in">
            <h2 className="font-bold text-white text-base">⏱ Timer Settings</h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-blue-900/15 border border-blue-800/30 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mb-3">Auto-submits at zero.</p>
                ...
                {form.duration > 0 && (
                  <p className="text-xs text-blue-400 mt-2">
                    = {form.duration >= 60 ? Math.floor(form.duration / 60) + 'h ' : ''}{form.duration % 60 > 0 ? form.duration % 60 + 'm' : ''}
                  </p>
                )}
              </div>
              <div className="bg-purple-900/15 border border-purple-800/30 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 mb-3">0 = manual close only.</p>
                <div className="flex items-center gap-2">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setStep(0)} className="flex-1 py-2.5 bg-white/8 hover:bg-white/12 rounded-xl text-sm transition-colors">← Back</button>
              <button type="button" onClick={() => setStep(2)} className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-500 rounded-xl text-sm font-semibold transition-colors">Next: Proctoring →</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="bg-gray-800/60 border border-white/8 rounded-2xl p-6 space-y-4 fade-in">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-white text-base">🕵️ Proctoring</h2>
              <div className="flex gap-2">
                <button type="button" onClick={enableAll} className="text-xs px-3 py-1.5 bg-blue-900/50 hover:bg-blue-800 border border-blue-700 text-blue-300 rounded-lg transition-colors">All On</button>
                <button type="button" onClick={disableAll} className="text-xs px-3 py-1.5 bg-white/8 hover:bg-white/12 rounded-lg transition-colors text-gray-300">All Off</button>
              </div>
            </div>
              ...
              {PROCTOR_KEYS.map(key => {
                ...
                return (
                  <div key={key} onClick={() => toggle(key)}
                    ...
                    <div className={`w-10 h-5.5 rounded-full flex items-center shrink-0 mt-0.5 transition-colors pt-0.5 ${on ? 'bg-blue-600' : 'bg-gray-600'}`} style={{height:'22px'}}>
                      <div className={`w-4 h-4 bg-white rounded-full mx-0.5 shadow transition-transform ${on ? 'translate-x-4' : 'translate-x-0'}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-semibold ${on ? 'text-white' : 'text-gray-300'}`}>{m.icon} {m.label}</p>
            ...
            {activeCount > 0 ? (
              <div className="bg-amber-900/15 border border-amber-800/40 rounded-xl p-4 text-xs text-gray-300 space-y-1">
                ...
                {form.proctoring.fullScreenForce && <p>⛶ Fullscreen required</p>}
                <p className="text-red-400 font-medium">⚠️ 3 violations → auto-submit with 0 marks</p>
              </div>
            ) : (
              <div className="bg-white/5 border border-white/8 rounded-xl p-3 text-center text-gray-400 text-xs">No proctoring active</div>
            )}
            <div className="flex gap-2">
              <button type="button" onClick={() => setStep(1)} className="flex-1 py-2.5 bg-white/8 hover:bg-white/12 rounded-xl text-sm transition-colors">← Back</button>
              <button type="button" onClick={() => setStep(3)} className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-500 rounded-xl text-sm font-semibold transition-colors">Next: Select Paper →</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4 fade-in">
            <div className="bg-gray-800/60 border border-white/8 rounded-2xl p-6">
              <h2 className="font-bold text-white text-base mb-4">📚 Question Paper</h2>
              {selectedPaper ? (
                ...
                <div className="border-2 border-dashed border-white/10 rounded-xl p-8 text-center">
                  ...
                  <p className="text-gray-400 text-sm mb-4">No question paper selected yet</p>
                  <button type="button"
                    onClick={() => navigate('/faculty/questions', { state: { selectMode: true } })}
            ...
            <div className="bg-gray-800/60 border border-white/8 rounded-2xl p-5 space-y-2 text-sm">
              <h3 className="font-semibold text-white text-xs uppercase tracking-wide text-gray-400 mb-3">Exam Summary</h3>
              {[
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setStep(2)} className="flex-1 py-2.5 bg-white/8 hover:bg-white/12 rounded-xl text-sm transition-colors">← Back</button>
