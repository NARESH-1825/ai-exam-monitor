  ...
  return (
    <div className="bg-white/4 border border-slate-700/20 rounded-xl p-4 space-y-3 fade-in">
      <h5 className="text-sm font-semibold text-blue-300">
        {idx !== null ? `Edit Question ${idx + 1}` : "New MCQ Question"}
      </h5>
      <textarea
        rows={2}
      ...
      <div className="grid grid-cols-2 gap-2">
        {form.options.map((opt, i) => (
          <div key={opt.id} className="flex gap-2 items-center">
            <span className="text-blue-400 font-bold text-xs w-4">
              {opt.id}.
            </span>
            <input
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={onCancel}
          className="px-4 py-1.5 bg-white/8 hover:bg-white/12 rounded-lg text-xs transition-colors"
        >
          Cancel
        </button>
      </div>
...
const PaperDrawer = ({
  ...
  const [mode, setMode] = useState(
    paper ? (isSelectMode ? "detail" : "detail") : "edit",
  );
  ...
  const handleSave = async () => {
    ...
    try {
      let saved;
      if (paper?.id) {
        const { data } = await api.put(`/faculty/papers/${paper.id}`, {
          title,
          questions,
        });
        saved = data.paper;
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end md:items-center md:justify-center">
      ...
      />
      <div
        className="relative z-10 w-full flex flex-col bg-gray-900
            ...
            <>
              ...
              <div className="space-y-2">
                {(paper.questions || []).map((q, i) => (
                  <div
                    key={i}
                    className="bg-white/[0.025] border border-slate-700/20 rounded-xl p-3"
                  >
              ...
              <div>
                ...
                <div className="space-y-1.5 mt-2 max-h-48 md:max-h-64 overflow-y-auto">
                  {questions.map((q, i) => (
                    <div
                      key={i}
                      className="bg-white/4 rounded-lg p-2.5 flex gap-2 items-start"
                    >
                      <span className="text-blue-400 font-bold text-xs w-5 shrink-0">
                        {i + 1}.
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-white truncate">{q.text}</p>
                        <p className="text-xs text-gray-500">
                          ✓ {q.answer} · {q.difficulty} · {q.marks}m
                        </p>
                      </div>
                      <div className="flex gap-1 shrink-0">
                        <button
                          ...
                          className="px-1.5 py-1 bg-blue-900/50 hover:bg-blue-800 rounded text-xs"
                        >
                          ✏️
                        </button>
                        <button
                          ...
                          className="px-1.5 py-1 bg-red-900/50 hover:bg-red-800 rounded text-xs"
                        >
                          🗑
                        </button>
                      </div>
        ...
        <div className="border-t border-slate-700/20 px-5 py-4 shrink-0 flex gap-2">
          {isSelectMode && paper ? (
            <button
              ...
              className="flex-1 py-2.5 bg-green-600 hover:bg-green-500 rounded-xl font-semibold text-sm transition-colors"
            >
              ✅ Select This Paper
            </button>
          ) : mode === "detail" && paper ? (
            <>
              <button
                onClick={() => setMode("edit")}
                className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-500 rounded-xl font-semibold text-sm transition-colors"
              >
                ✏️ Edit
              </button>
              <button
                onClick={() => onDelete(paper.id)}
                className="px-5 py-2.5 bg-red-700 hover:bg-red-600 rounded-xl text-sm transition-colors"
              >
                🗑
              </button>
            </>
            ...
            <>
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 rounded-xl font-semibold text-sm transition-colors"
              >
                {saving
                  ? "⏳ Saving..."
                  : paper?.id
              ...
              <button
                onClick={onClose}
                className="px-5 py-2.5 bg-white/8 hover:bg-white/12 rounded-xl text-sm transition-colors"
              >
                Cancel
              </button>
            </>
...
const QuestionBank = () => {
  ...
  const handleSelect = (paper) =>
    navigate("/faculty/exam-config", { state: { selectedPaper: paper } });
  ...
  const filteredPapers = useMemo(
    () =>
      papers.filter((p) => {
        ...
        const matchDate =
          !searchDate || (p.createdAt || "").startsWith(searchDate);
  ...
  return (
    <DashboardLayout
      title={isSelectMode ? "Select Question Paper" : "Question Bank"}
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        ...
        {(searchTitle || searchDate) && (
          <button
            ...
            className="px-3 py-2.5 bg-white/8 hover:bg-white/12 rounded-xl text-sm transition-colors text-gray-300"
          >
            ✕
          </button>
        )}
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">
                    📅 {fmt(paper.createdAt)} · {paper.createdBy}
                  </p>
                </div>
              ...
              {paper.updatedAt && paper.updatedAt !== paper.createdAt && (
                <p className="text-yellow-400 text-xs mb-2">
                  ✏️ Modified {fmt(paper.updatedAt)}
                </p>
              )}
                ...
                {!isSelectMode && (
                  <>
                    <button
                      ...
                      className="px-3 py-1.5 bg-blue-900/50 hover:bg-blue-800 rounded-lg text-xs transition-colors"
                    >
                      ✏️
                    </button>
                    <button
                      onClick={() => handleDelete(paper.id)}
                      className="px-3 py-1.5 bg-red-900/50 hover:bg-red-800 rounded-lg text-xs transition-colors"
                    >
                      🗑
                    </button>
                  </>
